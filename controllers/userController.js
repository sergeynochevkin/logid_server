  ...
  async driver_registration(req, res, next) {
    try {
      ...
      let userInfo = await UserInfo.findOne({
        where: { uuid: user_info_uuid },
      });
      await limit_service.check_account_activated(language, userInfo.id);

      let userData = await userService.registration(
        user_id,
        user_info_uuid,
        email.toLowerCase(),
        password,
      ...
      let userAppSettingsDefaultList = [
        {
          name: "sms_messaging",
          value: country === "russia" ? false : false,
          role: "both",
          managing_by: "user",
        },
        {
        ...
        {
          name: "can_see_new_orders",
          value: true,
          role: "driver",
          managing_by: "supervisor",
        },
        {
          name: "can_take_order",
          value: true,
        ...
        {
          name: "can_finish_order",
          value: true,
          role: "driver",
          managing_by: "supervisor",
        },
        {
          name: "can_set_order_as_disrupted",
          value: false,
          role: "driver",
          managing_by: "supervisor",
        },
      ...
      userAppSettingsDefaultList = userAppSettingsDefaultList.filter(
        (el) => el.role === role || el.role === "both"
      );
      ...
      for (const setting of userAppSettingsDefaultList) {
        await UserAppSetting.findOrCreate({
          where: {
            ...
            managing_by: setting.managing_by,
          },
        });
      }
  ...
  async fast_registration(req, res, next) {
    try {
      ...
      if (!errors.isEmpty()) {
        return next(
          ApiError.badRequest(
            translateService.setNativeTranslate("english", {
              russian: ["Ошибка валидации"],
              english: ["Validation error"],
      ...
      userData.user.userInfoId = user_info.id;
      ...
      await Subscription.create({
        ...
        paid_to,
      }); // plan by promocodes && validity date promocode from link or from form!
      await UserAppState.create({ userInfoId: user_info.id });
      await UserAppLimit.create({ userInfoId: user_info.id });
      await LimitCounter.create({ userInfoId: user_info.id });
      ...
      let userAppSettingsDefaultList = [
        {
          name: "sms_messaging",
          value: country === "russia" ? true : false,
          role: "both",
          managing_by: "user",
        },
        {
    ...
    try {
      ...
      return res.send(
        translateService.setNativeTranslate(language, {
          ...
          spanish: [
            "Se ha enviado un enlace de activación de cuenta nueva a",
            email,
          ],
          turkish: [
            "Yeni bir hesap aktivasyon bağlantısı şu adrese gönderildi:",
            email,
          ],
