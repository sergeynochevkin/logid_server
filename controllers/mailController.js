...
const { Order, UserInfo, Offer, Transport } = require('../models/models')
...
class MailController {
    async send(req, res, next) {
        try {
            ...
            //Когда массив добавить опциональность
            if (Array.isArray(orderId)) {
            ...
            if (mailFunction === 'new_order') {
                ...
                mover_text = `Это автоматическое уведомление, ответ не будет прочитан`
                ...
                allMembers_text = `Это автоматическое уведомление, ответ не будет прочитан`
            ...
            if (mailFunction === 'order_type') {
                ...
                if (offers.length !== 0) {
                    ...
                    member_text = `Это автоматическое уведомление, ответ не будет прочитан`
            ...
            if (mailFunction === 'order_status') {
                ...
                // неподача, незагрузка массовая обработка не планируется
                    ...
                    // отправка по массовой обработке но письма по каждому заказу каждоу кто делал предложение, это логично, так как активности будут происходить в разное время и от разных пользователей
                    if (Array.isArray(orderId)) {
                        member_text = `Это автоматическое уведомление, ответ не будет прочитан`
