...
module.exports = async function () {
    ...
    for (const row of plans) {
        ...
        if (checkItem) {
            SubscriptionPlan.update({ name: row.name, value: row.value, bage: row.bage, comment: row.comment, country: row.country, frequency: row.frequency, period: row.period, price: row.price }, { where: { [Op.and]: { plan_id: row.plan_id, country: row.country } } })
        } else {
            SubscriptionPlan.create({ plan_id: row.plan_id, name: row.name, value: row.value, bage: row.bage, comment: row.comment, country: row.country, frequency: row.frequency, period: row.period, price: row.price, country: row.country })
        }
    }
    for (const row of options) {
        let checkItem = await SubscriptionOption.findOne({ where: { [Op.and]: { option_id: row.option_id, country: row.country } } })
        if (checkItem) {
            SubscriptionOption.update({ name: row.name, comment: row.comment, value: row.value, role: row.role, limit: row.limit, type: row.type }, { where: { [Op.and]: { option_id: row.option_id, country: row.country } } })
        } else {
            SubscriptionOption.create({ option_id: row.option_id, name: row.name, comment: row.comment, value: row.value, role: row.role, limit: row.limit, type: row.type, country: row.country })
        }
    }
